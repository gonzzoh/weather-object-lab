...
getCity = function(param) {
    let place = param.city.name
    let placeLat = param.city.coord.lat
    let placeLon = param.city.coord.lon 
    console.log(`The coordinates of ${place} are ${placeLat} latitude and ${placeLon} longitude`);
}
...
forecast = function(param) {
    let place = param.city.name
    let placeClim = param.list[32].weather[0].description
    let placeWind = param.list[32].wind.speed
    console.log(`Today at ${place} the skies are ${placeClim} with a wind speed of ${placeWind}`);
}
forecast(evanstonWeather)


timeAndTemperature = function(param) {
    let place = param.city.name
    let placeTemp = param.list[5].main.temp
    console.log(`Today at ${place} at 9am; the temperature will be ${placeTemp}`)
}   
timeAndTemperature(evanstonWeather)

stPatWeather = function(param) {
    let place = param.city.name
    let windSpeed = ((param.list[14].wind.speed)*2.26394)
